...
| R-05 | Escritura abierta en equipments, loans y returns | `docs/supabase/004_sincronizacion.sql` concede INSERT/UPDATE a `anon` con políticas `using (true)` para la sincronización | Cualquiera con la anon key puede crear o modificar préstamos de otro usuario o cambiar el estado de un equipo | Aceptado (Sprint 6–8) | Políticas RLS por `auth.uid()` (requiere Supabase Auth) o escrituras mediante funciones RPC que validen al usuario |
