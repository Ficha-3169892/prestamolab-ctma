...
| R-05 | Escritura abierta en equipments, loans y returns | `docs/supabase/004_sincronizacion.sql` concede INSERT/UPDATE a `anon` con políticas `using (true)` para la sincronización; `006_inventario.sql` agrega DELETE en `equipments` (HU-12) y `007_actividades.sql` abre `activities` (HU-11) | Cualquiera con la anon key puede crear o modificar préstamos de otro usuario, cambiar el estado de un equipo, borrar equipos sin préstamos o crear, editar y borrar actividades | Aceptado (Sprint 6–8) | Políticas RLS por `auth.uid()` (requiere Supabase Auth) o escrituras mediante funciones RPC que validen al usuario |
